          ...
          <motion.div initial={{ opacity: 0, y: 80, scale: 0.95, x: 20 }}
            ...
            <div className="p-4 relative overflow-hidden" style={{
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => setOpen(false)} className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-dark-400 hover:text-white hover:bg-white/10 transition-all">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {suggestions.map((s) => (
                    <button key={s.label} onClick={s.action}
                      className="text-xs px-3 py-2.5 rounded-xl glass-card text-dark-300 hover:text-white hover:border-primary-500/30 transition-all text-left flex items-center gap-1.5">
                      {s.label}
              ...
              <div className="px-4 pb-2 flex flex-wrap gap-1.5">
                {leadQuestions[leadStage].options.map((opt) => (
                  <button key={opt} onClick={() => sendMessage(opt)}
                    className="text-xs px-3 py-1.5 rounded-full glass-card text-dark-300 hover:text-white hover:border-primary-500/30 transition-all">
                    {opt}
                  </button>
                ))}
              </div>
            ...
            <div className="p-4 border-t border-white/5">
              <div className="flex items-center gap-2 rounded-2xl bg-white/5 border border-white/10 p-1.5 focus-within:border-primary-500/50 transition-all">
                <input value={input} onChange={(e) => setInput(e.target.value)}
                  ...
                  className="flex-1 bg-transparent px-3 py-2 text-sm text-white placeholder-dark-400 outline-none" />
