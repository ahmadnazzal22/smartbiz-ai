...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed left-0 top-0 h-screen w-64 bg-dark-900/95 backdrop-blur-xl border-r border-[#DBEAFE] flex flex-col z-50
        transition-transform duration-300
        lg:translate-x-0
        ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        <div className="p-5 border-b border-[#DBEAFE] flex items-center justify-between">
          <NavLink to="/dashboard" className="flex items-center gap-2.5 group" onClick={() => setMobileOpen(false)}>
            ...
            <div>
              <span className="text-sm font-bold text-slate-900">Smart</span>
              <span className="text-sm font-bold gradient-text">Biz</span>
              <span className="text-sm font-bold text-slate-900"> AI</span>
            </div>
          </NavLink>
          <button onClick={() => setMobileOpen(false)} className="lg:hidden text-dark-400 hover:text-slate-900">
            <X className="w-5 h-5" />
          </button>
        </div>
            ...
            <NavLink key={to} to={to} onClick={() => setMobileOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                  isActive
                    ? 'text-slate-900 border shadow-sm'
                    : 'text-dark-400 hover:text-slate-900 hover:bg-[#F0F4FF]'
        ...
        <div className="p-3 border-t border-[#DBEAFE] space-y-2">
          ...
          <div className="flex items-center gap-2.5 px-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-slate-900 truncate font-medium">{user?.name || 'User'}</p>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-30 bg-dark-900/95 backdrop-blur-xl border-t border-[#DBEAFE] flex items-center justify-around px-2 py-1 safe-area-bottom">
