      ...
      {lead && (
        <>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30 }}
            className="fixed right-0 top-0 h-full w-full max-w-lg bg-dark-900/95 backdrop-blur-xl border-l border-[#DBEAFE] z-50 overflow-y-auto shadow-2xl">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 ${
                      isHot ? 'bg-red-500/10 text-red-400' : isWarm ? 'bg-yellow-500/10 text-yellow-400' : 'bg-[#F0F4FF] text-dark-400'
                    }`}>
                ...
                <button onClick={onClose}
                  className="w-8 h-8 rounded-lg bg-[#F0F4FF] flex items-center justify-center hover:bg-[#F0F4FF] transition-all">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#F0F4FF] rounded-xl p-3 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-dark-300 bg-[#F0F4FF] rounded-xl p-4">{lead.notes}</p>
              </div>

              {/* AI Recommendation */}
              <div className={`rounded-xl p-4 ${isHot ? 'bg-red-500/5 border border-red-500/10' : isWarm ? 'bg-yellow-500/5 border border-yellow-500/10' : 'bg-[#F0F4FF] border border-[#DBEAFE]'}`}>
              ...
              {bookings.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {bookings.map((b, i) => (
                      <div key={i} className="flex items-center justify-between bg-[#F0F4FF] rounded-xl p-3">
                        <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-dark-200 bg-[#F0F4FF] hover:bg-[#F0F4FF] transition-all">
