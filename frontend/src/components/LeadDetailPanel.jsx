      ...
      {lead && (
        <>
          ...
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 30 }}
            className="fixed right-0 top-0 h-full w-full max-w-lg bg-dark-900/95 backdrop-blur-xl border-l border-cream-dark z-50 overflow-y-auto shadow-2xl">
            
            <div className="p-6 space-y-6">
              {/* Header */}
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-brand-500 to-accent-500 flex items-center justify-center text-lg font-bold text-white">
                    {lead.avatar}
                  </div>
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">{lead.name}</h2>
                    <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium mt-1 ${
                      isHot ? 'bg-red-500/10 text-red-400' : isWarm ? 'bg-yellow-500/10 text-yellow-400' : 'bg-surface-alt text-dark-400'
                    }`}>
                ...
                <button onClick={onClose}
                  className="w-8 h-8 rounded-lg bg-surface-alt flex items-center justify-center hover:bg-surface-alt transition-all">
              ...
              <div>
                ...
                <p className="text-sm text-dark-300 bg-surface-alt rounded-xl p-4">{lead.notes}</p>
              </div>

              {/* AI Recommendation */}
              <div className={`rounded-xl p-4 ${isHot ? 'bg-red-500/5 border border-red-500/10' : isWarm ? 'bg-yellow-500/5 border border-yellow-500/10' : 'bg-surface-alt border border-cream-dark'}`}>
              ...
              {bookings.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2">
                    {bookings.map((b, i) => (
                      <div key={i} className="flex items-center justify-between bg-surface-alt rounded-xl p-3">
                        <div>
                          <p className="text-sm text-slate-900">{b.service}</p>
              ...
              <div className="flex gap-3 pt-2">
                <button className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-slate-900 transition-all hover:shadow-lg"
                  style={{ background: 'linear-gradient(135deg, #7c3aed, #6366f1)' }}>
                  <Phone className="w-4 h-4" /> Call
                </button>
                <button className="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-sm font-semibold text-dark-200 bg-surface-alt hover:bg-surface-alt transition-all">
