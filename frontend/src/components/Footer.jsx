          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex gap-3">
              {[Mail, Github, Twitter].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-xl glass-card flex items-center justify-center text-dark-400 hover:text-slate-900 hover:border-brand-500/30 transition-all hover:scale-110">
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              ...
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a href="#" className="text-sm text-dark-400 hover:text-slate-900 transition-colors hover:translate-x-1 inline-block">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
